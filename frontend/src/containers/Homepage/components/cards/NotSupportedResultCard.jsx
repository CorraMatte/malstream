...
const NotSupportedResultCard = () => {
  const {data, isIdle, isLoading, isError} = useStatsTotalResults('not_supported');
  const {
    data: dataTotal,
    isIdle: isIdleTotal,
          ...
          (isLoading || isIdle || isIdleTotal || isLoadingTotal) ? <LoadingSpinner /> :
            <>
              <div className="dashboard__progressbar-card-container">
                <h5 className="dashboard__progressbar-card-title dashboard__booking-total-title--green">
                  {data?.count}
                </h5>
              </div>
              <h5 className="dashboard__progressbar-card-description">File not analyzed by the sandbox</h5>
...
export default NotSupportedResultCard;
